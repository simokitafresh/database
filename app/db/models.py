...
class PriceRaw(Base):
    """Immutable raw OHLCV observations by source."""
    ...
    __table_args__ = (
        ...
        sa.CheckConstraint(
            "open > 0 AND high > 0 AND low > 0 AND close > 0",
            name="ck_prices_raw_positive_ohlc",
        ),
        sa.CheckConstraint(
            "low <= LEAST(open, close)",
            name="ck_prices_raw_low_le_open_close",
        ),
        sa.CheckConstraint(
            "GREATEST(open, close) <= high",
            name="ck_prices_raw_open_close_le_high",
        ),
        sa.CheckConstraint("volume >= 0", name="ck_prices_raw_volume_nonneg"),
        Index("idx_prices_raw_symbol_date", "symbol", "date"),
        Index(
    ...
    date = sa.Column(sa.Date, nullable=False)
    source = sa.Column(sa.String(20), nullable=False)
    open = sa.Column(sa.Numeric(18, 6), nullable=False)
    high = sa.Column(sa.Numeric(18, 6), nullable=False)
    low = sa.Column(sa.Numeric(18, 6), nullable=False)
    close = sa.Column(sa.Numeric(18, 6), nullable=False)
    volume = sa.Column(sa.BigInteger, nullable=False)
    fetched_at = sa.Column(sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now())
    consensus_close = sa.Column(sa.Numeric(18, 6), nullable=True)
...
class CorporateEvent(Base):
    ...
    dividend_amount = sa.Column(sa.Numeric(18, 8), nullable=True)
    split_ratio = sa.Column(sa.Numeric(18, 8), nullable=True)
    currency = sa.Column(sa.String(3), nullable=True, default="USD")
    ex_date = sa.Column(sa.Date, nullable=True)
    record_date = sa.Column(sa.Date, nullable=True)
    pay_date = sa.Column(sa.Date, nullable=True)
    confirmed_at = sa.Column(sa.DateTime(timezone=True), nullable=True)
    source_count = sa.Column(sa.Integer, nullable=True)
    sources_json = sa.Column(postgresql.JSONB, nullable=True)
