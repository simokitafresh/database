...
async def list_symbols(session: AsyncSession, active: bool | None = None) -> Sequence[Any]:
    ...
    if active is None:
        sql = "SELECT symbol, name, exchange, currency, is_active, first_date, last_date FROM symbols ORDER BY symbol"
        result = await session.execute(text(sql))
    else:
        sql = "SELECT symbol, name, exchange, currency, is_active, first_date, last_date FROM symbols WHERE is_active = :active ORDER BY symbol"
