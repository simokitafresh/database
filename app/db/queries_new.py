...
async def _get_coverage(session: AsyncSession, symbol: str, date_from: date, date_to: date) -> dict:
    ...
    # TextClause への単純なキーワード引数での bindparams は、既存パラメータ定義がない場合に
    # ArgumentError を起こすため、execute 側にパラメータ辞書を渡す方式に変更
...
async def get_prices_resolved(
    ...
    out: List[dict] = []
    sql = text("SELECT * FROM get_prices_resolved(:symbol, :date_from, :date_to)").bindparam(
        bindparam("symbol", String),
        bindparam("date_from", Date),
        bindparam("date_to", Date)
    )
    for s in symbols:
        # 同上：bindparams ではなく実行時に辞書で束縛
        res = await session.execute(
...
async def list_symbols(session: AsyncSession, active: bool | None = None) -> Sequence[Any]:
    ...
    result = await session.execute(
        text(LIST_SYMBOLS_SQL).bindparam(bindparam("active", String)), 
        {"active": active}
    )
