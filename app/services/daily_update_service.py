...
from app.services.raw_price_pipeline import run_raw_price_pipeline
...
class DailyUpdateService:
        ...
        try:
            ...
            if request.dry_run:
                return CronDailyUpdateResponse(
                    status="success",
                    message=(
                        "Dry run completed. Would process "
                        f"{len(all_symbols)} symbols in batches of {batch_size}"
                    ),
            ...
            raw_price_result = await self._run_raw_price_pipeline(
                request, all_symbols, date_from, date_to
            )
            ...
            return CronDailyUpdateResponse(
                ...
                raw_price_pipeline=raw_price_result,
            )
    ...
    async def _run_adjustment_check(self, request):
        if request.check_adjustments and settings.ADJUSTMENT_CHECK_ENABLED:
            try:
                return await self.check_adjustments(auto_fix=request.auto_fix_adjustments)
            except Exception as e:
    ...
    async def _run_raw_price_pipeline(self, request, symbols, date_from, date_to):
        if not request.run_raw_price_pipeline:
            return None
        try:
            result = await run_raw_price_pipeline(self.session, symbols, date_from, date_to)
            await self.session.commit()
            return {"status": "success", **result}
        except Exception as e:
            await self.session.rollback()
            logger.error(f"Raw price pipeline failed: {e}")
            return {"status": "error", "error": str(e)}
    ...
    async def _determine_economic_date_range(self, request):
        ...
        if date_from is None:
            ...
            stmt = select(func.min(EconomicIndicator.date), func.max(EconomicIndicator.date)).where(
                EconomicIndicator.symbol == "DTB3"
            )
