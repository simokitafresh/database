...
**対応完了 (2025-12-03):** `auto_fetch=false` 時の制限緩和（100銘柄・200,000行）を実装いたしました。
...
| ご質問 | 回答 |
|--------|------|
| **DB取得時の銘柄数制限緩和** | **対応完了**（`auto_fetch=false` 時は100銘柄・200,000行まで可能） |
| **レート制限の適用範囲** | DB読み出しのみの場合は **適用されません** |
| **Fetch Job API のフロー** | パターンB（DBへの取り込みのみ、データは別途取得） |
| **バルクReadエンドポイント** | `auto_fetch=false` で対応完了 |
