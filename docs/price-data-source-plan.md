...
| **Phase 2** | 多数決監視+入力確定ゲート: (a)毎晩EODHD/Tiingo突合+不一致警報cron (b)月初入力確定ゲート(全コアシンボルの前月最終営業日行存在検証→不在ならpending+警報) (c)確定済みシグナル上書き防止(recalculate時にconfirmed月はSKIP) | 忍者(cmd_3688) | ★今ここ |
