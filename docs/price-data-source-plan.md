...
| **Phase 2** | 多数決監視+入力確定検証: EODHD/Tiingo突合cron(JST 08:00/17:00)+月初入力確定検証+不一致警報 | 忍者(cmd_3688) | **完了(2026-07-05)** |
| **Phase 3** | 生値+自前調整への移行: §6のアーキテクチャ実装。DM-Signal APIの互換性維持 | 忍者(cmd_3689) | ★今ここ |
...
| 予算 | 月$100以内(殿裁定)。**採用構成: EODHD EOD+Intraday All World Extended($29.99/mo)**+Tiingo(無料)+Alpaca IEX(無料) |
| プライマリ最終確定 | **EODHD確定**(殿裁定2026-07-05「それでいこう」)。EOD+Intradayプランはリバランサーアプリにも活用可能 |
| EODHDプラン | **EOD+Intraday All World Extended**($29.99/mo=$299.90/年)。EOD+配当/分割+調整済み値+イントラデイ。Calendar API(upcoming)は不要(未確定イベントは調整に使わない) |
...
| 月末最終営業日の価格が翌日まで未確定 | 7/1 01:00 JST cronで6/30 close取得するがEODHD更新は市場終了後数時間 | 前日データで誤計算 | **入力確定ゲート**: 全コアシンボルの前月最終営業日行が存在するか検証。不在→pending維持+警報 |
...
| ソース間の最終営業日判定不一致 | 半日取引(感謝祭翌日等) | 月末closeが1日ずれる | 多数決: 2/3ソース一致する日を採用 |
| UTC/JST日跨ぎ | 米国市場close 16:00 ET = 翌日05:00 JST | 01:00 JSTのcronが市場close前に走る | 月初夕方再計算(17:00 JST)で確定。01:00は暫定 |
| **複合: 配当落ち日=月末 × UTC/JST日跨ぎ** | 7/3 TECL/XLU事件の再現パターン | 最も危険。入力未到着+配当調整タイミング不一致 | 入力確定ゲート + 夕方再計算 + 多数決の三重防御 |

**設計原則(殿裁定2026-07-03 12:52):** 月初シグナルは前月最終営業日のopen/close確定値で計算し月内不変であるべき。

**Calendar API(upcoming配当/分割)について:** 未確定イベントは調整に使わない。ex_date通過済みの確定イベントのみ自前調整に使用する。Calendar APIは現プラン(EOD+Intraday)には含まれず、Fundamentals($59.99+)が必要だが、不要。

## 8. APIキー取得ステップバイステップガイド
