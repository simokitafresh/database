...
- [ ] **開始条件**: `app/services/fetch_worker.py`を開く
- [ ] **作業内容**: L43-44の`async with session.begin():`行を削除
- [ ] **終了条件**: 該当行が削除され、インデントエラーがない
- [ ] **検証方法**: `python -m py_compile app/services/fetch_worker.py`でエラーなし

### Task 1.2: fetch_worker.pyのインデント修正
- [ ] **開始条件**: Task 1.1完了
- [ ] **作業内容**: L45以降のインデントを1レベル左にシフト（try:ブロック全体）
- [ ] **終了条件**: tryブロックが正しいインデントレベルになっている
- [ ] **検証方法**: エディタのインデント表示で確認

### Task 1.3: トランザクションテストコード作成
- [ ] **開始条件**: `tests/unit/`ディレクトリにアクセス
- [ ] **作業内容**: `test_fetch_worker_transaction.py`を新規作成
- [ ] **コード内容**:
```python
import pytest
...
```
- [ ] **終了条件**: テストファイルが作成され、構文エラーなし
- [ ] **検証方法**: `pytest tests/unit/test_fetch_worker_transaction.py -v`
...
- [ ] **開始条件**: `app/db/queries.py`を開く
- [ ] **作業内容**: L230付近、`ensure_coverage_with_auto_fetch`関数内に日付検証追加
- [ ] **コード内容**:
```python
...
```
- [ ] **終了条件**: 条件分岐が正しく追加されている
- [ ] **検証方法**: 構文チェック成功

### Task 2.2: 部分データ処理の追加
- [ ] **開始条件**: Task 2.1完了
- [ ] **作業内容**: 同じく`ensure_coverage_with_auto_fetch`内、日付調整時の処理追加
- [ ] **コード内容**:
```python
...
- [ ] **作業内容**: `test_date_boundary.py`を新規作成
- [ ] **コード内容**:
```python
import pytest
from datetime import date
...
```
- [ ] **終了条件**: テストが作成され実行可能
- [ ] **検証方法**: `pytest tests/unit/test_date_boundary.py -v`
...
- [ ] **開始条件**: `app/services/fetcher.py`を開く
- [ ] **作業内容**: L58付近、最初の`yf.download()`呼び出しに`auto_adjust=True`追加
- [ ] **変更前**: 
```python
df = yf.download(
    symbol,
...
```
- [ ] **終了条件**: パラメータが追加されている
- [ ] **検証方法**: grepで確認 `grep -n "auto_adjust" app/services/fetcher.py`

### Task 3.2: fetcher.pyのauto_adjust追加（2箇所目）
- [ ] **開始条件**: Task 3.1完了
- [ ] **作業内容**: L79付近、フォールバックの`tk.history()`呼び出しを確認
- [ ] **確認内容**: `auto_adjust=True`が既に設定されているか確認、なければ追加
- [ ] **終了条件**: 両方のYahoo Finance呼び出しでauto_adjustが明示的
- [ ] **検証方法**: 該当箇所の目視確認

### Task 3.3: fetch_worker.pyのtickerも修正
- [ ] **開始条件**: `app/services/fetch_worker.py`を開く
- [ ] **作業内容**: L162付近、`ticker.history()`の呼び出しを確認
- [ ] **変更内容**: `auto_adjust=True`が明示的に設定されているか確認、なければ追加
- [ ] **終了条件**: パラメータ確認完了
- [ ] **検証方法**: 該当箇所の確認

### Task 3.4: YFinance警告テスト作成
- [ ] **開始条件**: `tests/unit/`ディレクトリ
- [ ] **作業内容**: `test_yfinance_warnings.py`を新規作成
- [ ] **コード内容**:
```python
import logging
import pytest
...
```
- [ ] **終了条件**: テスト作成完了
- [ ] **検証方法**: `pytest tests/unit/test_yfinance_warnings.py -v`
...
- [ ] **開始条件**: Task 6.1完了
- [ ] **作業内容**: 全修正ファイルのフォーマット
- [ ] **コマンド**: `black app/services/fetch_worker.py app/db/queries.py app/services/fetcher.py`
- [ ] **終了条件**: フォーマット完了
- [ ] **検証方法**: `black --check app/`
