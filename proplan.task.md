...
#### 受け入れ基準（AC）
- [ ] `API_MAX_SYMBOLS_LOCAL` が設定可能
- [ ] `API_MAX_ROWS_LOCAL` が設定可能
- [ ] 環境変数でオーバーライド可能
- [ ] デフォルト値が適切に設定されている
...
#### 受け入れ基準（AC）
- [ ] `auto_fetch=true` 時は 10銘柄制限
- [ ] `auto_fetch=false` 時は 100銘柄まで許可
- [ ] エラーメッセージに制限値を含める
- [ ] 既存のテストが通過する
...
#### 受け入れ基準（AC）
- [ ] `auto_fetch=true` 時は 50,000行制限
- [ ] `auto_fetch=false` 時は 200,000行まで許可
- [ ] エラーメッセージに制限値を含める
- [ ] 既存のテストが通過する
