以下は、“コード作成に専念するエンジニアリングLLM”にそのまま渡せるようにした、超細分化・テスト可能なタスクリストです。
各タスクは小さく・単一関心・開始と終了が明確で、受け入れ基準(AC) と 成果物（作る/変更するファイル）を必ず持ちます。
進捗記入用に status/owner/start/end/notes を空で用意しています（任意で埋めて使ってください）。

> 前提技術（固定）：Python 3.12.10、FastAPI、SQLAlchemy(2.x, asyncpg)、Pydantic v2、Alembic、Typer、pytest、yfinance、pandas/numpy、gunicorn+uvicorn。
外部通信を伴うテストはすべてモックで実施。DB統合テストはdocker-compose前提のコードは書くが、実行は不要（ここではコード作成のみ）。
...
> 使い方：このYAMLを別LLMに渡し、タスクID順（depends_on を満たしつつ）に実装・テストを進めます。
...
yfinance.download は pytest-mock で差し替え、DataFrameを返す。

DB呼び出しは app/db/queries.py の関数をモックしてI/O契約だけ検証。

スリープやバックオフは time.sleep / asyncio.sleep をモック。


型と検証

PriceRowOut.last_updated は datetime（tzあり）。date は datetime.date。

settings.CORS_ALLOW_ORIGINS 空ならCORS未設定、CSVなら分割。


SQL/DDLの検証（DB不要）

SQLAlchemy の CreateTable(model.__table__).compile(dialect=postgresql.dialect()) でDDL文字列を生成し、PRIMARY KEY, CHECK, FOREIGN KEY の断片を in 判定。


メトリクスの精度

小数誤差は pytest.approx で許容（例：1e-12）。


行上限の推定

API_MAX_ROWS に到達するケースはモックで行数だけを制御して 413 を誘発。
...
1. 0–1–2（雛形/設定/モデル） → 3（Alembic） → 4（スキーマ）


2. 5（サービス：normalize→resolver→metrics→upsert→fetcher→lock）


3. 6–7（API枠→DBクエリ層） → 8（CLI骨格） → 9–10（ログ/配線）


4. 12–14（制限/交差/再試行） → 15–16（E2E疑似→CI yml）
...
このタスクリストは、“コードとテストの作成”だけで進められるよう最小粒度に分けてあります。
DB・ネットワークに依存する部分はすべてモック前提で完結し、のちに実インフラに接続しても契約と境界条件が崩れない構成です。
